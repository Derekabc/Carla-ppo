...
class CarlaRouteEnv(gym.Env):
    """
        In this CARLA environment the agent is tasked in completing a random
        route between random two points on the map each episode. The routes
        are generated by the GlobalRoutePlanner, which uses A* internally.
        Can work on any map, but only tested on Town07.
